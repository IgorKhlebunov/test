...
static const char *s_makeWords = "words";
static const char *s_makeChecksum = "checksum";
...
int main(int argc, char **argv)
{
    ...
    if (s_makeWords == work) {
        ...
        std::function<std::string(const std::vector<char>&, const std::string&)> f =
            ...
            uint32_t count = 0;
            for (size_t offset = text.find(s); offset != std::string::npos;
                 offset = text.find(s, offset + s.length())) {
                ++count;
            }

            std::stringstream result;
            result << "Count of \"" << s << "\" = " << count;
        ...
        std::function<std::string(const std::vector<char>&)> f = [] (const std::vector<char>& v) {
            ...
            static const std::vector<uint32_t> crc32Table = {
            ...
            while (i != v.end())
                checksum = crc32Table.at((checksum ^ *i++) & mask) ^ (checksum >> shift);
